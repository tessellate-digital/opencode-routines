...
router.post('/api/:triggerId', async (c) => {
  ...
  let valid = false;
  try {
    valid = provided.length > 0 && token.length > 0 &&
      timingSafeEqual(Buffer.from(provided), Buffer.from(token));
  } catch { /* length mismatch = invalid */ }

  if (!valid) return c.json({ detail: 'Invalid token' }, 401);

  const routine = db.prepare('SELECT * FROM routines WHERE id = ? AND enabled = 1').get(trigger.routine_id) as RoutineRow | undefined;
  if (!routine) return c.json({ detail: 'Routine not found or disabled' }, 404);
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, 'api', ?, 'pending', ?, ?)
  `).run(runId, routine.id, trigger.id, prompt, JSON.stringify(text ? { text } : {}), now);

  executor.startRun(runId, routine, prompt).catch(err => console.error(`Run ${runId} error:`, err));
...
router.post('/github/:triggerId', async (c) => {
  ...
  if (!eventHeader) return c.json({ detail: 'Missing X-GitHub-Event header' }, 400);
  ...
  if (!matched) {
    return c.json({ status: 'skipped', reason: `Event ${eventHeader} not in filter` });
  }
  ...
  for (const [k, v] of Object.entries(metadata)) {
    if (k !== 'event') contextLines.push(`  ${k}: ${v}`);
  }
  ...
  `).run(runId, routine.id, trigger.id, prompt, JSON.stringify(metadata), now);
