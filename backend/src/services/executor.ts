...
function parseOpencodeEvent(line: string): StreamEvent | null {
  ...
  if (etype === 'text') {
    ...
    if (tokens || cost) {
      return { type: 'status', data: `--- done (tokens: ${tokens.total ?? 0}, cost: $${cost.toFixed(4)}) ---\n` };
    }
  } else if (etype === 'error') {
    const err = (event.error as Record<string, unknown>) ?? {};
    const msg = ((err.data as Record<string, unknown>)?.message as string) ?? (err.name as string) ?? 'unknown error';
...
export class Executor {
  ...
  private async prepareWorkspace(routine: RoutineRow): Promise<string> {
    ...
    if (routine.repository) {
      ...
      if (fs.existsSync(path.join(repoDir, '.git'))) {
        await execFileAsync('git', ['-C', repoDir, 'fetch', 'origin'], { timeout: 120_000 });
        await execFileAsync('git', ['-C', repoDir, 'checkout', routine.branch], { timeout: 30_000 });
        await execFileAsync('git', ['-C', repoDir, 'pull', '--ff-only'], { timeout: 120_000 });
      } else {
        await execFileAsync('git', ['clone', '-b', routine.branch, routine.repository, repoDir], { timeout: 300_000 });
      }
      return repoDir;
    }
   ...
   * the CLI.
   */
  private buildPromptContext(routine: RoutineRow, userPrompt: string): { context: string; fullPrompt: string } {
    const now = new Date();
    ...
    `).get(routine.id) as { status: string; started_at: string | null; finished_at: string | null } | undefined;
    ...
    if (lastRun) {
      ...
      if (lastRun.started_at) lines.push(`Last run started: ${lastRun.started_at}`);
      if (lastRun.finished_at) lines.push(`Last run finished: ${lastRun.finished_at}`);
    } else {
  ...
  async startRun(runId: string, routine: RoutineRow, prompt: string): Promise<void> {
    ...
    const { context, fullPrompt } = this.buildPromptContext(routine, prompt);

    // Persist the context in metadata so the frontend can show it
    const existingMeta = db.prepare('SELECT metadata FROM runs WHERE id = ?').get(runId) as { metadata: string } | undefined;
    let meta: Record<string, unknown> = {};
    try { meta = JSON.parse(existingMeta?.metadata ?? '{}'); } catch { /* ignore */ }
    ...
    eventBus.broadcast('run_started', { run_id: runId, routine_id: routine.id, status: 'running' });
    ...
    try {
      ...
      try {
        ...
        db.prepare(`UPDATE runs SET status = 'failed', stderr = ?, finished_at = ? WHERE id = ?`)
          .run(`Workspace preparation failed: ${err}`, new Date().toISOString(), runId);
        eventBus.broadcast('run_finished', { run_id: runId, routine_id: routine.id, status: 'failed' });
        queue.push(null);
      ...
      if (routine.model) cmd.push('--model', routine.model);
      else if (config.opencodeModel) cmd.push('--model', config.opencodeModel);
      cmd.push('--format', 'json');

      let proc: ReturnType<typeof spawn>;
      try {
        proc = spawn(cmd[0], cmd.slice(1), { cwd: workdir, env, stdio: ['ignore', 'pipe', 'pipe'] });
      } catch (err) {
        db.prepare(`UPDATE runs SET status = 'failed', stderr = ?, finished_at = ? WHERE id = ?`)
      ...
      const startIdleTimer = () => {
        ...
        idleTimer = setTimeout(() => {
          console.warn(`[executor] Run ${runId}: opencode process idle for ${IDLE_TIMEOUT_MS / 1000}s after step_finish — killing.`);
          proc.kill('SIGTERM');
          setTimeout(() => {
            if (this.processes.has(runId)) proc.kill('SIGKILL');
          }, 5000);
        }, IDLE_TIMEOUT_MS);
      };
      ...
      const currentRow = db.prepare('SELECT status FROM runs WHERE id = ?').get(runId) as { status: string } | undefined;
      const alreadyCancelled = currentRow?.status === 'cancelled';

      const MAX = 1024 * 1024;
      if (alreadyCancelled) {
        // Preserve cancelled status, just persist output
        db.prepare(`UPDATE runs SET stdout = ?, stderr = ?, exit_code = ? WHERE id = ?`)
          .run(
            stdoutBuf.join('\n').slice(0, MAX),
        ...
        db.prepare(`UPDATE runs SET stdout = ?, stderr = ?, exit_code = ?, status = ?, finished_at = ? WHERE id = ?`)
          .run(
            stdoutBuf.join('\n').slice(0, MAX),
            stderrBuf.join('').slice(0, MAX),
            exitCode,
        ...
        eventBus.broadcast('run_finished', { run_id: runId, routine_id: routine.id, status: finalStatus, exit_code: exitCode });
      }
    } catch (err) {
  ...
  async cancelRun(runId: string): Promise<void> {
    ...
    db.prepare(`UPDATE runs SET status = 'cancelled', finished_at = COALESCE(finished_at, ?) WHERE id = ? AND status IN ('pending', 'running')`)
      .run(new Date().toISOString(), runId);
